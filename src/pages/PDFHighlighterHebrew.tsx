...
const PDFHighlighter = () => {
  ...
  const loadPDFJS = () => {
    console.log('loadPDFJS נקרא');
    if (!window.pdfjsLib) {
      console.log('טוען PDF.js...');
      const script = document.createElement('script');
      script.src = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js';
      script.onload = () => {
        console.log('PDF.js נטען בהצלחה!');
        window.pdfjsLib.GlobalWorkerOptions.workerSrc = 
          'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js';
        console.log('Worker נקבע בהצלחה');
      };
      script.onerror = () => {
        console.error('שגיאה בטעינת PDF.js');
      };
      document.head.appendChild(script);
    } else {
      console.log('PDF.js כבר טעון');
    }
  };

  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    console.log('handleFileUpload נקרא');
    ...
    if (file && file.type === 'application/pdf') {
      ...
      console.log('הקובץ לא תקין או לא נבחר');
    }
  };

  const loadPDF = async (data: ArrayBuffer) => {
    console.log('loadPDF נקרא, גודל data:', data.byteLength);
    try {
      if (!window.pdfjsLib) {
        console.error('PDF.js לא נטען!');
        alert('שגיאה: ספריית PDF.js לא נטענה. אנא רענן את הדף.');
        return;
      }
      console.log('יוצר loadingTask...');
      const loadingTask = window.pdfjsLib.getDocument({ data });
      console.log('ממתין ל-PDF...');
      const pdf = await loadingTask.promise;
      console.log('PDF נטען בהצלחה! מספר עמודים:', pdf.numPages);
      pdfDocRef.current = pdf;
      setTotalPages(pdf.numPages);
      setCurrentPage(1);
    } catch (error) {
      console.error('שגיאה בטעינת PDF:', error);
      alert('שגיאה בטעינת הקובץ: ' + error);
    }
  };
